# Hi there, I'm Bo-Da Li!

## About Me

I'm a **Full-Stack & AI Engineer** from Taiwan, passionate about building digital products that solve real-world problems. I specialize in creating high-performance, user-centric applications by integrating frontend, backend, and artificial intelligence technologies.

My journey in software development has led me to work on diverse projects ranging from healthcare education systems to AI-powered virtual streamer platforms. I believe in the power of technology to create meaningful impact and improve user experiences at scale.

### Current Focus
- Developing AI-integrated applications using modern web technologies
- Exploring innovative solutions in healthcare technology and interactive AI systems
- Building scalable full-stack applications with emphasis on user experience

### Education & Background
- **National Yunlin University of Science and Technology** - Department of Information Management (2022-2026)
- Location: Taiwan

## Tech Stack & Skills

### Frontend Development
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white)

- React 19 with modern hooks and patterns
- TypeScript for type-safe development
- Tailwind CSS v4 for responsive design

### Backend Development
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)

- Python for AI integration and backend services
- Spring Boot for enterprise-level applications
- MySQL for relational database management

### AI & Advanced Tools
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

- **OpenAI API & Realtime API** - Building conversational AI experiences
- **RAG (Retrieval-Augmented Generation)** - Implementing context-aware AI systems
- **FAISS** - Vector database for efficient similarity search
- **LINE Bot Development** - Creating interactive chatbot solutions
- **WebSocket** - Real-time bidirectional communication
- **Cloudflare** - Edge computing and deployment optimization

## Featured Projects

I've worked on several impactful projects that demonstrate my ability to identify pain points and deliver effective solutions:

- **AI Virtual Streamer** - Real-time interactive AI character with voice and visual responses
- **HIV Healthcare Education System** - Comprehensive digital platform improving patient care and education
- **E-Scooter Management System** - Full-stack solution for urban mobility management

Check out my complete portfolio for detailed case studies and live demos!

## What I'm Learning

- Advanced AI integration patterns and prompt engineering
- Financial technology applications with AI
- Stock market analysis and algorithmic trading
- Scalable microservices architecture

## Let's Connect!

- **Portfolio**: [https://v016604z.github.io/portfolio/](https://v016604z.github.io/portfolio/)
- **LinkedIn**: [https://linkedin.com/in/李柏達-liboda-09a659280](https://linkedin.com/in/李柏達-liboda-09a659280)
- **Email**: v016604@gmail.com
---

*Open to collaboration opportunities and innovative projects. Feel free to reach out!*
